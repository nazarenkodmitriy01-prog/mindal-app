...
// ============================================================
// Распознавание табака по фото банки (вкладка "Полка") — через
// Claude API (Anthropic). Ключ живёт только здесь, на сервере —
// в браузер он никогда не попадает.
// ============================================================
async function recognizeTobacco(imageBase64, mimeType) {
  ...
  const prompt = 'На фото — банка кальянного табака. Определи производителя '
    + '(бренд) и название вкуса по этикетке. Ответь СТРОГО одним JSON-объектом, '
    + 'без пояснений и без markdown-разметки, в формате '
    + '{"manufacturer": "...", "flavor": "..."} на русском языке. '
    + 'Если не можешь уверенно определить какое-то поле — поставь туда null.';
  ...
  if (data.error) {
    const err = new Error((data.error && data.error.message) || 'anthropic_error');
...
app.post('/tobacco/recognize', async (req, res) => {
  let body;
  try {
    body = JSON.parse(req.body);
  } catch (e) {
  ...
  if (!body.imageBase64) return res.status(400).json({ ok: false, error: 'missing_image' });
  try {
    const result = await recognizeTobacco(body.imageBase64, body.mimeType);
    res.json({ ok: true, manufacturer: result.manufacturer, flavor: result.flavor });
  } catch (e) {
    if (e.code === 'not_configured') {
      return res.status(400).json({
        ok: false, error: 'not_configured',
        message: 'Распознавание не настроено: на сервере не задан ANTHROPIC_API_KEY.',
      });
    }
...
app.get('/iiko/abc', async (req, res) => {
  if (!iiko.configured()) return res.status(400).json({ error: 'iiko_not_configured' });
  const from = req.query.from;
  const to = req.query.to;
  if (!from || !to) return res.status(400).json({ error: 'missing_from_to', message: 'Укажите ?from=YYYY-MM-DD&to=YYYY-MM-DD' });
  try {
    const result = await iiko.fetchAbcAnalysis(from, to);
    res.json({ ok: true, ...result });
  } catch (e) {
    res.status(500).json({ ok: false, error: String(e.message || e) });
  }
});
app.get('/iiko/diag/dangerous', async (req, res) => {
